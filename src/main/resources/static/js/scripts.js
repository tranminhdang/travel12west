(function($) {
	...
	$.fn.imageCover = function() {
		$(this)
				.each(
						function() {
							var self = $(this), image = self.find('img'), heightWrap = self
									.outerHeight(), widthImage = image
									.outerWidth(), heightImage = image
									.outerHeight();
	...
	function masonry() {
		$('.awe-masonry').each(
				function() {
					var $gallery = $(this), $galleryItem = $gallery
							.children('.awe-masonry__item');
					setTimeout(function() {
						$gallery.masonry({
	...
	function slider() {
		var paginationSlider = [ '<i class="fa fa-caret-left"></i>',
		...
		$('.product-slider-wrapper')
				.each(
						function() {
							var el = $(this), slider = el
									.find('.product-slider'), sliderThumb = el
									.find('.product-slider-thumb');
							slider
									.owlCarousel({
										autoPlay : false,
										slideSpeed : 500,
										...
										afterAction : function(el) {
											...
											sliderThumb.find(".owl-item")
													.removeClass("synced").eq(
															current).addClass(
															"synced")
											if (sliderThumb.data("owlCarousel") !== undefined) {
												"use strict";
												var center = function(number) {

													var slidethumnailvisible = sliderThumb
															.data("owlCarousel").owl.visibleItems;
													var num = number;
													var found = false;
				...
				function() {
					...
					priceslider.slider({
						...
						slide : function(event, ui) {
							...
							priceslider.siblings('.price_slider_amount').find(
									'.from').text('$' + values[0]);
							priceslider.siblings('.price_slider_amount').find(
									'.to').text('$' + values[1]);
						}
					...
					priceslider.siblings('.price_slider_amount').find('.from')
							.text('$' + values[0]);
					priceslider.siblings('.price_slider_amount').find('.to')
							.text('$' + values[1]);
				});
	}
	...
	function seasonHover() {
		var season = $('.travelling-tabs__time').find('.season'), seasonItem = season
				.find('.item'), month = $('.travelling-tabs__time').find(
				'.month'), monthItem = month.find('.item');
		seasonItem.each(function() {
			...
			monthItem.hover(function() {
				var $this = $(this), filterSeason = $this.data('season');
				$this.addClass('hover-active');
					...
					.magnificPopup(
							{
								...
								callbacks : {
									ajaxContentAdded : function() {
										...
										$('.flight-popup__map-content-list')
												.mdMapExtend(
														{
												...
												.tabs(
														{
															...
															activate : function(
																	event, ui) {
																$(
																		'.flight-popup__grid-wrapper .image-wrap')
																		.imageCover();
																if ($(
																		'.flight-popup__grid-wrapper',
																		ui.newPanel).length
																		&& !$(
																				'.flight-popup__grid-wrapper',
																				ui.newPanel)
	...
	function stickyMenu() {
		var $fixed = $('.header-page__inner'), offsetTop = $fixed.offset().top, scrollTop = $(
				window).scrollTop(), height = $fixed.outerHeight(), limitTop = offsetTop
				+ height;
		$fixed.data('scrollTop', scrollTop);
		$(window)
				.scroll(
						function() {
							var scrollTop = $(window).scrollTop(), beforeTop = $fixed
									.data('scrollTop');

							if (beforeTop < scrollTop) {
								if (!$fixed.hasClass('header-page__fixed')) {
									if (scrollTop > limitTop) {
										var transform = 'translateY(-' + height
												+ 'px)';
										$fixed
									...
									var y = $fixed.data('y'), offsetScroll = beforeTop
											- scrollTop, newTop = (y + offsetScroll) < height ? (y - offsetScroll)
											: height, newTransform = 'translateY(-'
											+ newTop + 'px)';
									$fixed.css({
								...
								if ($fixed.hasClass('header-page__fixed')) {
									var y = $fixed.data('y'), offsetScroll = beforeTop
											- scrollTop, newTop = (y - offsetScroll) > 0 ? (y - offsetScroll)
											: 0, newTransform = 'translateY(-'
											+ newTop + 'px)';
									...
									if (scrollTop < offsetTop) {
										$fixed
												.removeClass('header-page__fixed');
										$fixed
												.css(
														{
	...
	$(window)
			.on(
					'load resize',
					function() {
						var dataResponsive = $('.navigation')
								.data('responsive'), windowWidth = window.innerWidth, windowHeight = window.innerHeight, headerHeight = $(
								'#header-page').height();
						...
						if (windowWidth <= dataResponsive) {
							...
							$('.awe-navigation-responsive').height(
									windowHeight - headerHeight);
							$('.search-box .form-search').css('right', '-60px');
							subToggle();
							submenuBack();
						} else {
							$('.navigation').insertAfter(
									'.header-page__inner .logo');
							$('.navigation').removeClass(
									'awe-navigation-responsive').addClass(
									'awe-navigation');
							$(document).find('.navigation').css('height',
									'auto');
							$('.search-box .form-search').css('right', '0');
							$('.submenu-toggle, .back-mb').remove();
						}
						$('.search-box .form-search').width(
								$('#header-page .container').width());
					});
	$('.toggle-menu-responsive').on(
			'click',
			function(evt) {
				evt.preventDefault();
				$(this).toggleClass('toggle-active');
				$(document).find('.awe-navigation-responsive').toggleClass(
	...
	$(document)
			.ready(
					function() {
						...
						if ($('#header-page nav').hasClass(
								'awe-navigation-hamburger')) {
							$('#header-page nav').attr('data-responsive',
									100000);
						}

						if (isMobile.iOS()) {
							$('.awe-parallax, .awe-static').addClass(
									'fix-background-ios');
						}
						...
						$('#page-wrap.dark').parent('body').addClass(
								'body-dark');
						...
						if ($('.contact-page__map').length) {
							if (window.innerWidth >= 992) {
								$('.contact-page__map').height(
										$('.contact-page__form').outerHeight());
							}
						...
						if (isMobile.any()) {
							...
							$(document).on(
									...
									function() {
										...
										$('.minicart-body').removeClass(
												'cart-toggle');
										$('.toggle-minicart').removeClass(
												'toggle-active');
									});
							...
							$('.search-box').hover(
									function() {
										$('.search-box .form-search').addClass(
												'form-active');
									},
									function() {
										$('.search-box .form-search')
